...
/** Persists the home action list: user order and last-used. */
class ActionStore(context: Context) {
    ...
    /** Built-in actions in the user's saved order (unseen items appended in default order). */
    fun actions(): List<Action> {
        val all = DefaultActions.list
